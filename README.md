...
The installer detects macOS/Linux and `amd64`/`arm64`, verifies the published SHA-256 checksum, and installs to `~/.local/bin`. Pass `--version v0.4.1 --dir "$HOME/bin"` to pin a version or directory.

Alternatives:

```bash
brew install galjos/odh/odh
sudo apt install ./odh_v0.4.1_linux_amd64.deb   # from GitHub Releases
