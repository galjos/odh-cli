...
The installer detects macOS/Linux and `amd64`/`arm64`, verifies the published SHA-256 checksum, and installs to `~/.local/bin`. Pass `--version v0.6.0 --dir "$HOME/bin"` to pin a version or directory.

Alternatives:

```bash
brew install galjos/odh/odh
sudo apt install ./odh_v0.6.0_linux_amd64.deb   # from GitHub Releases
