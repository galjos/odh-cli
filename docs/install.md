...
```bash
curl -fsSL https://raw.githubusercontent.com/galjos/odh-cli/main/scripts/install.sh | sh -s -- --version v0.1.11
```
...
sh install-odh.sh --version v0.1.11 --dir "$HOME/bin"
```
...
```bash
ODH_VERSION=v0.1.11 ODH_INSTALL_DIR="$HOME/bin" sh scripts/install.sh
```
...
```bash
go install github.com/galjos/odh-cli/cmd/odh@v0.1.11
```
