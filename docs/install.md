...
```bash
curl -fsSL https://raw.githubusercontent.com/galjos/odh-cli/main/scripts/install.sh | sh -s -- --version v0.1.2
```
...
sh install-odh.sh --version v0.1.2 --dir "$HOME/bin"
```
...
```bash
ODH_VERSION=v0.1.2 ODH_INSTALL_DIR="$HOME/bin" sh scripts/install.sh
```
...
```bash
go install github.com/galjos/odh-cli/cmd/odh@v0.1.2
```
