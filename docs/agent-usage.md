...
This source supports `--from`/`--to`/`--today`, `--near`/`--radius`, `--search`, `--type`, `--limit`, `--include-expired`, and `--zone-id`/`--area`. It **rejects** `--road` and `--type bike` with exit code 2, because announcements name the road only inside free text and carry no cycle tag — cycle notices are filed under the same tags as any other closure or roadwork, so a tag filter would drop real matches. Do not work around a rejection by dropping the constraint from the answer: either switch to `--source odh`, or narrow with `--near` or `--search` and say which you used.

`--zone-id` and `--area` work here by **geographic inference**, not by reading a field: the announcement's coordinates are matched against a committed table of ~1100 coordinates whose zone the Mobility feed recorded, and it matches when the nearest of them is within 2.0 km and carries a requested zone. Announcements farther away, or without coordinates, are unassignable — excluded and counted in a warning. The inferred zone is never written into `zone_id`, `zone`, or `zone_it`. Surface the warning the response emits, and say "in the area around X" rather than "the province filed this under zone X". Municipality aliases such as `--area kaltern` narrow only to the zone here, which the response also warns about; use `--search` when the answer must be about the village rather than the district.
...
- Area filter inferred by the Content API: "The bulletin records carry coordinates but no zone, so `odh traffic --source content --area pustertal` selected the announcements nearest to coordinates the province historically tagged as that zone, within 2 km. These are notices in that area, not notices the province filed under that zone, and the CLI reported N it could not place at all."
