...
```bash
clawhub skill publish "$(pwd)/skills/open-data-hub-cli" --version 0.1.17 --clawscan-note "Uses network access only through the odh CLI to query public Open Data Hub endpoints. Declares odh as a required binary and provides an OpenClaw Go installer hint."
```
