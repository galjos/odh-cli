...
Run the same checks CI does before tagging, `golangci-lint` included. `go vet`
does not report unused functions, so a helper left behind by a refactor passes
locally and fails the Lint job after the tag is already pushed:
