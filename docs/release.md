...
Releases are built from Git tags named `v*`, for example `v0.1.9`.
...
```bash
VERSION=v0.1.9 scripts/build-release.sh
```

Cross-compile by setting `GOOS` and `GOARCH`:

```bash
VERSION=v0.1.9 GOOS=linux GOARCH=amd64 scripts/build-release.sh
```
