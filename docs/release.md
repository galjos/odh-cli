...
Releases are built from Git tags named `v*`, for example `v0.2.5`.
...
```bash
VERSION=v0.2.5 scripts/build-release.sh
```

Cross-compile by setting `GOOS` and `GOARCH`:

```bash
VERSION=v0.2.5 GOOS=linux GOARCH=amd64 scripts/build-release.sh
```
...
```bash
VERSION=v0.2.5 GOOS=linux GOARCH=amd64 scripts/build-deb.sh
```
...
```bash
gh release view v0.2.5 --repo galjos/odh-cli
