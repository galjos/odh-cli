...
Releases are built from Git tags named `v*`, for example `v0.4.2`.
...
```bash
VERSION=v0.4.2 scripts/build-release.sh
```

Cross-compile by setting `GOOS` and `GOARCH`:

```bash
VERSION=v0.4.2 GOOS=linux GOARCH=amd64 scripts/build-release.sh
```
...
```bash
VERSION=v0.4.2 GOOS=linux GOARCH=amd64 scripts/build-deb.sh
```
...
```bash
gh release view v0.4.2 --repo galjos/odh-cli
gh release download v0.4.2 --repo galjos/odh-cli --pattern SHA256SUMS --pattern 'odh_v0.4.2_darwin_arm64.tar.gz'
grep 'odh_v0.4.2_darwin_arm64.tar.gz' SHA256SUMS
shasum -a 256 odh_v0.4.2_darwin_arm64.tar.gz
gh attestation verify odh_v0.4.2_darwin_arm64.tar.gz --repo galjos/odh-cli
```
