...
Releases are built from Git tags named `v*`, for example `v0.3.0`.
...
```bash
VERSION=v0.3.0 GOOS=linux GOARCH=amd64 scripts/build-release.sh
```
...
```bash
VERSION=v0.3.0 GOOS=linux GOARCH=amd64 scripts/build-deb.sh
```
...
```bash
gh release view v0.3.0 --repo galjos/odh-cli
